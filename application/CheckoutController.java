...
public enum CheckoutController implements CleanupControl {
    ...
    private final String TERMS_MESSAGE_FILE = CustomerConstants.CURR_DIR+"\\resources\\terms.txt";
    private final String GOODBYE_FILE = CustomerConstants.CURR_DIR+"\\resources\\goodbye.txt";
	...
	class ProceedToCheckoutListener implements ActionListener,Controller {
        ...
        /**
         * This method populates the shipping billing window
         * with the default address data for this customer.
         * Retrieves the customer name and default shipping
         * and billing addresses from customer subsystem interface.
         * These values should have been loaded into the Customer
         * subsystem at login.
         */
        void populateScreen()  {
            SessionContext context = SessionContext.INSTANCE;
            /*
             * Get customer subsystem and read customer profile and 
             * default shipping and billing addresses here
             */
            ...
            shippingBillingWindow.setShippingAddress(custProfile.getFirstName() + " "+custProfile.getLastName(),
                                                     ...
                                                     defaultShipAddress.getState(),
                                                     defaultShipAddress.getZip());
            shippingBillingWindow.setBillingAddress(custName.getFirstName() + " "+custName.getLastName(),
        ...
        public void actionPerformed(ActionEvent evt) {
        	...
        	if(!loggedIn.booleanValue()) {
        	    ...
        	    LoginControl loginControl = 
        	        new LoginControl(shippingBillingWindow,
        	            			 cartItemsWindow,
                                     this);
	...
	class SelectShipButtonListener implements ActionListener {
        public void actionPerformed(ActionEvent evt) {
            ...
            ICustomerSubsystem cust = (ICustomerSubsystem)SessionContext.INSTANCE.get(CustomerConstants.CUSTOMER);
            
            try {
                List<IAddress> addresses = cust.getAllAddresses();
                List<String[]> addressesAsArrays = CustomerUtil.addrListToListOfArrays(addresses);
                shipAddrs.updateModel(addressesAsArrays);
                shipAddrs.setVisible(true);
            }
            catch(DatabaseException e){
                JOptionPane.showMessageDialog(shipAddressesWindow,                                                    
                          "Unable to locate customer addresses",
                          "Error", 
	...
	class ProceedFromBillingCheckoutListener implements ActionListener {
	       ICustomerSubsystem cust;
	        ...
	        public void actionPerformed(ActionEvent evt) {
	            ...
	            shippingBillingWindow.setVisible(false);
	            cust= (ICustomerSubsystem)SessionContext.INSTANCE.get(CustomerConstants.CUSTOMER);
				fullname = cust.getCustomerProfile().getFirstName()+" "+cust.getCustomerProfile().getLastName();
	            if(shippingBillingWindow.isNewShipAddress()){
	            	
	            	String[] addrFlds = shippingBillingWindow.getShipAddressFields();
	            	
	            	IAddress addr = cust.createAddress(addrFlds[0],addrFlds[1],addrFlds[2],addrFlds[3]);
	            	...
	            	if(rulesOk) {
	            		//do updates
	            		shippingBillingWindow.setAddressFields(new String[]{cleansedAddr.getStreet1(),
	            										cleansedAddr.getCity(),
	            										cleansedAddr.getState(),
	            										cleansedAddr.getZip()});
	        ...
	        void setupPaymentWindow() {
				...
				paymentWindow.setCredCardFields(ccAsArray[0],ccAsArray[1],ccAsArray[2],ccAsArray[3]);
	...
	class SelectAddressesListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent evt) {
        	...
        	if(selectedRow >= 0) {
                ...
                SessionContext context = SessionContext.INSTANCE;
                //get cust name from customer subsystem -- for now we use fake data
	...
	class SubmitFinalOrderListener implements ActionListener {
	       public void actionPerformed(ActionEvent evt) {
	        	...
	        	JOptionPane.showMessageDialog(finalOrderWindow,         									          
	        									          msg,
	        									          "E-Bazaar: Thank You", 
    ...
    public ActionListener getProceedFromBillingCheckoutListener(ShippingBillingWindow w){
